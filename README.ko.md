...
현재 이 프로젝트 개발과 로컬 Git 서비스에 사용 중인 서버는 다음과 같습니다:

| 항목 | 값 |
| :--- | :--- |
| 호스트 | `daystar-HP-Z840-Workstation` |
| Tailscale IP | `100.73.157.10` |
| 프로젝트 대시보드 | `http://100.73.157.10:8787/` |
| 로컬 Gitea | `http://100.73.157.10:3030/daystar/ReMux` |
| 로컬 Git remote | `http://100.73.157.10:3030/daystar/ReMux.git` |
| ServerDashBoard / LLM 위키 | `http://100.65.161.20:5001/#board` |

나중에 다른 장비에서 같은 Tailscale 네트워크로 접속할 때 이 정보를 기준으로
개발 서버와 로컬 Git 미러를 찾으면 됩니다. 운영 메모, LLM 위키 인제스트 규칙,
프로젝트/서버 간 사실 확인이 필요할 때는 ServerDashBoard를 확인합니다.
